...
-- deleting external table deletes the table ie metadata but records arent deleted 

IF OBJECT_ID('bronze.taxi_zone') IS NOT NULL 
DROP EXTERNAL TABLE bronze.taxi_zone

-- Create taxi_zone table -- create external table 
...
(
	...
	REJECT_VALUE=10,     -- can reject 10 records
	REJECTED_ROW_LOCATION='rejections/taxi_zone'    --new folder rejections inside nyc-taxi-data container
);
...
(
	...
	DATA_SOURCE=nyc_taxi_ext_data_src,
	FILE_FORMAT=csv_file_format   --use parser_version 2 because it is a big file
	-- no rejections for this file
);
...
-- dont provide ** to go inside folder for delta files
--synapse looks for delta log and fetches file
	DATA_SOURCE=nyc_taxi_ext_data_src,
