...
AS
BEGIN
	DECLARE @create_sql_stmt NVARCHAR(MAX),     -- variable to create cetas
		    @drop_sql_stmt NVARCHAR(MAX);       -- variable to drop cetas

	SET @create_sql_stmt=			            -- construct create cetas statment
	...
	EXEC sp_executesql @create_sql_stmt;           -- execute create cetas statement

	SET @drop_sql_stmt=			                   -- construct drop cetas statement
		'DROP EXTERNAL TABLE gold.trip_data_green_' + @year + '_' + @month;
	
	print(@drop_sql_stmt)

	EXEC sp_executesql @drop_sql_stmt;	            -- execute drop cetas statement
